...
class PostRepository
{
    ...
    {
        return (int)current($this->database->dbConnect()->query("SELECT COUNT(id) from post")->fetch());
    }
    ...
    {
        $result = $this->database->dbConnect()->prepare('UPDATE post SET title = :title, chapo = :chapo, post.user_id = :authorId, content = :content, last_update=NOW() WHERE post.id = :postId');
        $result->bindValue(':postId', $postId, PDO::PARAM_INT);
    ...
    {
        $result = $this->database->dbConnect()->prepare('DELETE FROM post WHERE post.id = :postId');
        $result->bindValue(':postId', $postId, PDO::PARAM_INT);
        $result->execute();
        if ($result->rowCount() > 0) {
            return true;
